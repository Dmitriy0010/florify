...
export function CheckoutPage() {
  ...
  const slots = rawSlots.filter(slot => slot.active)
                ...
                <div className="space-y-3">
                  ...
                  {slotsLoading ? (
                    ...
                    <div className="flex items-center gap-2 p-4 rounded-xl bg-amber-50 border border-amber-200">
                      ...
                      <p className="text-sm text-amber-700 font-medium">На сегодня свободных слотов нет. Попробуйте завтра или выберите самовывоз.</p>
                    </div>
                  ) : (
                    <RadioGroup
                      ...
                      {slots.map(slot => {
                        ...
                        return (
                          <label
                            key={slot.id}
                            className={cn(
                              "relative flex cursor-pointer rounded-xl border-2 bg-white p-4 shadow-sm transition-all hover:bg-slate-50",
                              isSelected
                                ? "border-[var(--color-brand)] bg-[var(--color-brand-light)]/10"
                                : "border-gray-100"
                            )}
                          >
                            ...
                            <span className="flex flex-1 items-center justify-between">
                              <span className="flex flex-col">
                                <span className={cn("block text-sm font-bold", isSelected ? "text-[var(--color-brand)]" : "text-gray-900")}>
                                  {slot.startTime.substring(0, 5)} — {slot.endTime.substring(0, 5)}
                                </span>
                                <span className="mt-1 text-xs text-gray-500">
                                  Свободно: {slot.remainingCapacity} из {slot.maxCapacity}
                                </span>
                              </span>
                              <CheckCircle2 className={cn(
                                "h-5 w-5 transition-opacity",
                                isSelected ? "text-[var(--color-brand)] opacity-100" : "opacity-0"
                              )} />
                            </span>
                          </label>
                        )
