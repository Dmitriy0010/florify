...
export function PaymentQRModal({ orderId, orderNumber, amount, qrData, onSuccess, onClose }: PaymentQRModalProps) {
  ...
  useEffect(() => {
    // Настройка WebSocket соединения (STOMP)
    const socket = new SockJS('/api/ws-orders')
    const stompClient = Stomp.over(socket)
    
    // Выводим ошибки SockJS в консоль
    socket.onerror = (err) => console.error('SockJS error:', err)
  ...
  const handleSimulate = async () => {
    try {
      ...
      await PaymentService.simulateSuccess(orderId)
      // Напрямую переводим в SUCCESS — в реальном мире это пришло бы через WebSocket
      setStatus('SUCCESS')
            ...
            <div className={cn(
              ...
              {status === 'SUCCESS' ? (
                ...
                <QRCodeSVG 
                  ...
                  includeMargin={false}
                  imageSettings={{
                    src: "https://sbp.nspk.ru/wp-content/themes/sbp/assets/img/logo.svg",
                    height: 30,
                    width: 30,
                    excavate: true,
                  }}
